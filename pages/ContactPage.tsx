...
const ContactPage: React.FC<ContactPageProps> = ({ developerId, developerName, onNavigate, session, showToast }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (success) {
        setIsSending(false);
        setIsSent(true);
        console.log(`Message sent to ${developerName} from ${email}: ${message}`);
      } else {
        setIsSending(false);
  ...
  const handleGoBack = () => {
    onNavigate('developer', { developerId, developerName });
  };
    ...
    <div className="bg-white py-20">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-2xl">
        <h1 className="text-4xl font-extrabold font-heading text-gray-900 text-center mb-4">
          Contact <span className="text-primary">{developerName}</span>
        </h1>
        ...
        {isSent ? (
          <div className="text-center bg-green-50 border border-green-200 text-green-800 p-8 rounded-lg">
            ...
            <p className="mb-6">{developerName} will get back to you shortly at {email}.</p>
            <button
