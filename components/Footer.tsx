...
const Footer: React.FC<FooterProps> = ({ onNavigate, showToast }) => {
  const socialLinks = [
    ...
    { name: 'Instagram', icon: <svg fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" className="w-5 h-5" viewBox="0 0 24 24"><rect width="20" height="20" x="2" y="2" rx="5" ry="5"></rect><path d="M16 11.37A4 4 0 1112.63 8 4 4 0 0116 11.37zm1.5-4.87h.01"></path></svg> },
    ...
    <footer className="bg-white text-gray-600 body-font">
      ...
      <div className="bg-gray-100">
        <div className="container mx-auto py-4 px-5 flex flex-wrap flex-col sm:flex-row">
          <p className="text-gray-500 text-sm text-center sm:text-left">© 2025 OWSCORP —
            <a href="https://twitter.com/owscorp" rel="noopener noreferrer" className="text-gray-600 ml-1" target="_blank">@OWSCORP</a>
          </p>
          <span className="inline-flex sm:ml-auto sm:mt-0 mt-2 justify-center sm:justify-start">
            {socialLinks.map(link => (
              <button key={link.name} onClick={() => showToast('Social media links are coming soon!', 'success')} className="ml-3 text-gray-500 hover:text-primary cursor-pointer">
                {link.icon}
              </button>
            ))}
          </span>
